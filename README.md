...
**这是什么？** 一个让本地视频也能显示在线弹幕的 API 服务器

**解决什么问题？** 在各类播放器（如 senplayer、forward、hills 等）中观看本地视频时，可以自动匹配并显示来自爱奇艺、优酷、腾讯、芒果TV、哔哩哔哩等平台的实时弹幕评论

**核心特点：**
- 🚀 **零门槛部署**：支持 Vercel/Netlify 一键部署，无需服务器和 NAS
- 🎯 **实时获取**：无需提前下载弹幕文件，直接从视频平台实时获取
- 🔌 **即插即用**：兼容弹弹play API 规范，支持主流播放器
- 🌐 **多平台支持**：涵盖爱奇艺、优酷、腾讯、芒果TV、B站、人人影视、韩剧TV、巴哈姆特等

一个人人都能部署的基于 JavaScript 的弹幕 API 服务器，支持爱优腾芒哔人韩巴弹幕直接获取，兼容弹弹play的搜索、详情查询和弹幕获取接口规范，并提供日志记录，支持vercel/netlify/edgeone/cloudflare/docker/claw等部署方式，不用提前下载弹幕，没有nas或小鸡也能一键部署。
